...
function JobCard({ children }: { children: ReactNode }) {
    return <article className='min-w-xl max-w-4xl border-2 p-4'>
        {children}
    </article>
}
// Header
function Header({ children }: { children: ReactNode }) {
    return <header aria-label='Job Title and Company' className="flex justify-between  align-center  text-2xl tracking-widesr">
        {children}
    </header>
}
...
function HeaderCompanyName({ children }: { children: ReactNode }) {
    return <p aria-label="Company Name" className="">{children}</p>
}

// Detail
function Detail({ children }: { children: ReactNode }) {
    return <section aria-label='Job Detail'><dl>{children}</dl></section>
}
